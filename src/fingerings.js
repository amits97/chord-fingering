...
function pressedNonBarreFrets(fingering) {
  return pressedFrets(fingering).filter(
    (fret) => !fingering.barre || fret !== fingering.barre.fret
  );
}

function minFret(a) {
  return Math.min(...pressedFrets(a));
}

function maxFret(a) {
  return Math.max(...pressedFrets(a));
}

function getFretRange(a) {
  return maxFret(a) - minFret(a);
}

function getFretRangeWithMin(a, n) {
  return Math.max(n, getFretRange(a));
}

function bassOctave(a) {
  return getOctave(a.positions[0].note);
}
...
function computeDifficulty(f, barrePenalty = 5) {
  let consideredPositions = f.positions.filter(
    (p) =>
      p.fret > 0 &&
      (!f.barre ||
  ...
  if (consideredPositions.length === 0) consideredPositions = f.positions;
  ...
  const fretDistances = a.slice(1).map((f1, i) => {
    const f2 = a[i];
    const d1 = distFromNut(f1),
      d2 = distFromNut(f2);
    return Math.abs(f1 - f2) > 1 ? Math.abs(d1 - d2) : Math.abs(d1 - d2);
  });
  ...
  const fretDistanceDifficulty = fretDistances.reduce(
    (acc, d) => acc + transform(d),
    0
  ); // for barre only chords
...
function anatomicalShape(f) {
  if (getFretRange(f) >= 3 && !f.barre) {
    const min = minFret(f);
    const atMin = f.positions
      .filter((p) => p.fret === min)
      .map((p) => p.stringIndex);
    const max = maxFret(f);
    const atMax = f.positions
      .filter((p) => p.fret === max)
      .map((p) => p.stringIndex);
...
function mutedStringsBetween(f, tuning) {
  ...
  const strummedStrings = tuning
    .map((s, i) => i)
    .slice(minString, maxString + 1);
  const mutedStrummedStrings = strummedStrings.filter(
    (i) => !f.positions.find((p) => p.stringIndex === i)
  );
  return mutedStrummedStrings.length;
}

function distinctNotesRatio(f) {
  const distinctNotes = f.positions
    .map((p) => p.note)
...
function findFingerings(
  ...
  // Restore bass string restriction for closed shapes
  let maxBassStringIndex = tuning.length - requiredNotes.length;
  if (!requiredNotes.includes(bass)) {
    maxBassStringIndex--;
  }
  let bassPositions = positions.filter(
    (p) => areNotesEqual(p.note, bass) && p.stringIndex <= maxBassStringIndex
  );
  ...
  for (let i = 0; i < bassPositions.length; i++) {
    ...
    let stringIndices = tuning
      .map((s, i) => i)
      .slice(bassPosition.stringIndex + 1);
    ...
    for (let j = 0; j < stringIndices.length; j++) {
      ...
      // Only add open string position if not already present and matches required note
      if (
        !stringPositions.some((p) => p.fret === 0) &&
        requiredNotes.includes(tuning[stringIndex])
      ) {
      ...
      fingeringsForThisBass = fingeringsForThisBass.flatMap((fingering) => {
        ...
        const reachablePositions = stringPositions.filter((position) => {
          return (
            position.fret === 0 ||
            nonZeroFrets.every(
              (fret) => Math.abs(position.fret - fret) <= MAX_FRET_DISTANCE
            )
          );
        });
  ...
  const isChordFull = (f) =>
    requiredNotes.every((note) =>
      f.find((position) => areNotesEqual(position.note, note))
    );
  const enoughFingers = (f) => {
    if (!f.barre) {
      ...
      return (
        pressedNonBarreFrets(f).length <= HUMAN_LEFT_HAND_PLAYING_FINGERS - 1
      );
    }
  };
  ...
  fingerings = fingerings
    ...
    .sort((a, b) => {
      const lessMutedStringsInBetween =
        mutedStringsBetween(a, tuning) - mutedStringsBetween(b, tuning);
      const notLessThanFourStrings = -(
        moreThanFourStrings(a) - moreThanFourStrings(b)
      );
      const lessRepeatingNotes = -(
        distinctNotesRatio(a) - distinctNotesRatio(b)
      );
